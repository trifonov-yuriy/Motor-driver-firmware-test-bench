...
def find_hex_files():
    ...
    # Ищем все файлы с расширением .hex
    hex_files = glob.glob(os.path.join(exe_dir, "*.hex"))
    
    return hex_files, exe_dir
...
def main():
    try:
        hex_files, search_dir = find_hex_files()
        if not hex_files:
            print("Файлы с расширением .hex не найдены в директории скрипта.")
            input()
            return       
        file_name = os.path.basename(hex_files[0])
        ...
        while(True):
            ...
            programmer.prog_motor_1(hex_files[0])
            programmer.prog_motor_2(hex_files[0])
            programmer.prog_motor_3(hex_files[0])
            programmer.prog_motor_4(hex_files[0])
    except Exception as e:
        print(e)
